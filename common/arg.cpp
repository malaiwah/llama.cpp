...
common_params_context common_params_parser_init(common_params & params,
    ...
    add_opt(
        common_arg({ "--models-preset-watch" }, { "--no-models-preset-watch" },
                   string_format(
                       "for router server, watch models_preset file for changes and reload automatically (default: %s)",
                       params.models_preset_watch ? "enabled" : "disabled"),
                   [](common_params & params, bool value) { params.models_preset_watch = value; })
            .set_examples({ LLAMA_EXAMPLE_SERVER })
            .set_env("LLAMA_ARG_MODELS_PRESET_WATCH"));
    add_opt(common_arg({ "--models-preset-watch-interval" }, "SECONDS",
                       string_format("for router server, interval in seconds for config file polling (default: %d)",
                                     params.models_preset_watch_interval),
                       [](common_params & params, int value) { params.models_preset_watch_interval = value; })
                .set_examples({ LLAMA_EXAMPLE_SERVER })
                .set_env("LLAMA_ARG_MODELS_PRESET_WATCH_INTERVAL"));
    add_opt(
        common_arg(
            { "--models-recycle-idle-seconds" }, "SECONDS",
            string_format(
                "for router server, idle timeout in seconds before recycling models with changed configs (default: %d)",
                params.models_recycle_idle_seconds),
            [](common_params & params, int value) { params.models_recycle_idle_seconds = value; })
            .set_examples({ LLAMA_EXAMPLE_SERVER })
            .set_env("LLAMA_ARG_MODELS_RECYCLE_IDLE_SECONDS"));
    add_opt(common_arg({ "--kv-cache-persist-path" }, "PATH",
                       "for router server, directory to save KV cache when models are unloaded (default: disabled)",
                       [](common_params & params, const std::string & value) {
                           params.kv_cache_persist_path = value;
                           if (!fs_is_directory(params.kv_cache_persist_path)) {
                               throw std::invalid_argument("not a directory: " + value);
                           }
                           if (!params.kv_cache_persist_path.empty() &&
                               params.kv_cache_persist_path[params.kv_cache_persist_path.size() - 1] !=
                                   DIRECTORY_SEPARATOR) {
                               params.kv_cache_persist_path += DIRECTORY_SEPARATOR;
                           }
                       })
                .set_examples({ LLAMA_EXAMPLE_SERVER })
                .set_env("LLAMA_ARG_KV_CACHE_PERSIST_PATH"));
    add_opt(common_arg({ "--no-kv-cache-on-unload" },
                       string_format("for router server, do not save KV cache when model is unloaded (default: %s)",
                                     params.kv_cache_on_unload ? "disabled" : "enabled"),
                       [](common_params & params) { params.kv_cache_on_unload = false; })
                .set_examples({ LLAMA_EXAMPLE_SERVER })
                .set_env("LLAMA_ARG_KV_CACHE_ON_UNLOAD"));
    add_opt(common_arg({ "--no-kv-cache-on-load" },
                       string_format("for router server, do not restore KV cache when model is loaded (default: %s)",
                                     params.kv_cache_on_load ? "disabled" : "enabled"),
                       [](common_params & params) { params.kv_cache_on_load = false; })
                .set_examples({ LLAMA_EXAMPLE_SERVER })
                .set_env("LLAMA_ARG_KV_CACHE_ON_LOAD"));
...
void common_params_add_preset_options(std::vector<common_arg> & args) {
    ...
    args.push_back(common_arg({ "pin" }, nullptr,
                              "in server router mode, do not unload this model if models_max is exceeded",
                              [](common_params &, const std::string &) { /* unused */ })
                       .set_env(COMMON_ARG_PRESET_PIN)
                       .set_preset_only());

    args.push_back(common_arg({ "kv-cache-persist-path" }, "PATH",
                              "in server router mode, directory to save KV cache when models are unloaded",
                              [](common_params &, const std::string &) { /* unused */ })
                       .set_env(COMMON_ARG_PRESET_KV_CACHE_PERSIST_PATH)
                       .set_preset_only());

    args.push_back(common_arg({ "no-kv-cache-on-unload" }, nullptr,
                              "in server router mode, do not save KV cache when model is unloaded",
                              [](common_params &, const std::string &) { /* unused */ })
                       .set_env(COMMON_ARG_PRESET_KV_CACHE_ON_UNLOAD)
                       .set_preset_only());

    args.push_back(common_arg({ "no-kv-cache-on-load" }, nullptr,
                              "in server router mode, do not restore KV cache when model is loaded",
                              [](common_params &, const std::string &) { /* unused */ })
                       .set_env(COMMON_ARG_PRESET_KV_CACHE_ON_LOAD)
                       .set_preset_only());
}
