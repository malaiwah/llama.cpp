...
int main(int argc, char ** argv) {
    ...
    // [AI] Load KV cache if configured
    if (!params.kv_cache_persist_path.empty()) {
        console::log("Loading KV cache from %s...\n", params.kv_cache_persist_path.c_str());
        if (ctx_cli.ctx_server.load_kv_cache(params.kv_cache_persist_path)) {
            console::log("KV cache loaded successfully\n");
        } else {
            console::log("No KV cache found or failed to load, starting fresh\n");
        }
    }
    ...
    // [AI] Save KV cache before exiting if configured
    if (!params.kv_cache_persist_path.empty()) {
        console::log("Saving KV cache to %s...\n", params.kv_cache_persist_path.c_str());
        if (ctx_cli.ctx_server.save_kv_cache(params.kv_cache_persist_path)) {
            console::log("KV cache saved successfully\n");
        } else {
            console::log("Failed to save KV cache\n");
        }
    }
